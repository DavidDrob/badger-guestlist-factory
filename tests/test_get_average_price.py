...
def test_get_price_covalent():
    ...
    response = requests.get(f"https://api.covalenthq.com/v1/pricing/historical_by_addresses_v2/1/USD/{token}/?quote-currency=USD&format=JSON&key={api_key}")
    parsed = json.loads(response.text)
    covalent_token_price = 1 / float((parsed['data'][0]['prices'][0]['price']))
    contract_token_price = factory_contract.getAverageTokenPrice(token, 1e6) / (10 ** 18)
